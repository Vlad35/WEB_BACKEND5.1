...
if ($_SERVER['REQUEST_METHOD'] == 'GET'){
	  ...
	  if (!empty($_COOKIE['save'])) {
		...
		$messages['saved'] = 'Вы зарегистрированы!';
		if (!empty($_COOKIE['pass'])) {
			$messages['passmessage'] = sprintf('Ваш логин: 
																					<strong>%s</strong>
																					Пароль: <strong>%s</strong> .
																					Войти:  <a href="login.php">Вход</a>',
	  ...
	  if ($errors['check']) {
		...
		$messages['bad_check'] = '<span class="error-text" style="color: red;">Вы не согласовали контракт!!</span>';
	  }
      ...
      session_start() && !empty($_SESSION['login'])){
		...
		printf('Вход с логином %s, Ваш %d', $_SESSION['login'], $_SESSION['uid']);
		}
	include('form.php');
	 }
else {
